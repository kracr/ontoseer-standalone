...
public class ODPDescription {
	...
	public List<String> findsimilarity(String st,String sts[]) {
		ODPElements odpElements = new ODPElements();
		JaroWinklerSimilarity similarity = new JaroWinklerSimilarity();

		List<String>ls1 = new ArrayList<String>();  // To store all the entities
		List<String>ls2 = new ArrayList<String>();
		List<Double>ls3 = new ArrayList<Double>();  // To store weightage of a particular string  
		List<Double>ls4 = new ArrayList<Double>();
		
		ls3.addAll(odpElements.findsimilarity(sts));
		for(int i=0;i<147;i++) {
			double d=0.0;
			d = similarity.similarityCheck(st, arr[i]); //calling the JaroWinkler similarity function
			ls.add(d);         // store similarity between opd discription and input string
			ls1.add(arr1[i]);
			ls5.add(IRI[i]);
